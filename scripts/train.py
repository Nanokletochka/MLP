...
if __name__ == "__main__":
    ...
    train_batch_generator = data.DataLoader(train_dataset, 500, shuffle=True, drop_last=False)

    # Инициализируем объект Dataset для обучения
    test_dataset = MNIST(is_train=False)

    # Инициализируем объект Dataloader для обучения
    test_batch_generator = data.DataLoader(test_dataset, 500, shuffle=True, drop_last=False)

    # Запись истории
    loss_train_history = []
    loss_test_history = []

    # Минимальное начальное качество для чекпоинта
    min_loss = 0.2
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_batch_generator, desc=f"Epoch {epoch + 1}/{epochs}", unit="batch") as pbar:
            for x_train, y_train in pbar:
                ...
                # Сохраняем наилучшее состояние модели
                if loss_value.item() < min_loss:
                    st = model.state_dict()
                    torch.save(st, "best.tar")
                    min_loss = loss_value.item()
